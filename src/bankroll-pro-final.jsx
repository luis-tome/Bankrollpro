...
export default function App() {
  ...
  async function handleCreateBR(isEdit){
    ...
    if(isEdit&&editBRTarget){
      ...
      const{data}=await supabase.from("profiles").insert({user_id:uid,name:brForm.name,sport:brForm.sport,bankroll:brv,unit_pct:parseFloat(brForm.unit_pct),trial_start:new Date().toISOString(),user_trial_start:earliestTrial,subscribed:false}).select().single();
  ...
  return (
    <div style={{background:"#f3f4f6",minHeight:"100vh",fontFamily:"-apple-system,'Segoe UI',sans-serif",color:"#111827",paddingBottom:100}} onTouchStart={swipeStart} onTouchEnd={swipeEnd}>
          ...
          <div style={{width:300,maxWidth:"85vw",background:"#fff",height:"100%",display:"flex",flexDirection:"column",padding:20,overflowY:"auto",boxShadow:"4px 0 24px rgba(0,0,0,.1)"}} onClick={e=>e.stopPropagation()}>
            ...
            {isInTrial && (
              <div style={{background:"#fef3c7",border:"1px solid #fde68a",borderRadius:10,padding:"12px",marginBottom:14,fontSize:12,color:"#92400e"}}>
                ...
                <div style={{marginBottom:8}}>Preço de lançamento garantido se subscreveres agora</div>
                <div style={{display:"flex",gap:6}}>
                  <a href={STRIPE_MONTHLY} target="_blank" rel="noreferrer" style={{flex:1,background:"#fff",border:"1px solid #fde68a",color:"#92400e",borderRadius:6,padding:"7px 0",fontSize:11,fontWeight:700,textAlign:"center",textDecoration:"none"}}>€{PROMO_MONTHLY}/mês</a>
                  <a href={STRIPE_ANNUAL} target="_blank" rel="noreferrer" style={{flex:1,background:"#111827",color:"#fff",borderRadius:6,padding:"7px 0",fontSize:11,fontWeight:700,textAlign:"center",textDecoration:"none"}}>€{PROMO_ANNUAL}/ano</a>
                </div>
              </div>
            )}
          ...
          <div style={{background:"#fff",borderRadius:16,padding:24,width:"100%",maxWidth:440,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)"}}>
            ...
            {!editBet && (
              <div style={{display:"flex",gap:8,marginBottom:4}}>
                <button style={{flex:1,padding:"8px",border:`1px solid ${formMode==="immediate"?sc.color:"#e5e7eb"}`,borderRadius:8,background:formMode==="immediate"?sc.color:"#f9fafb",color:formMode==="immediate"?"#fff":"#9ca3af",cursor:"pointer",fontSize:12,fontWeight:700}} onClick={()=>setFormMode("immediate")}>Resultado imediato</button>
                <button style={{flex:1,padding:"8px",border:`1px solid ${formMode==="pending"?sc.color:"#e5e7eb"}`,borderRadius:8,background:formMode==="pending"?sc.color:"#f9fafb",color:formMode==="pending"?"#fff":"#9ca3af",cursor:"pointer",fontSize:12,fontWeight:700}} onClick={()=>setFormMode("pending")}>Deixar pendente</button>
              </div>
            )}
            ...
            <div style={{display:"flex",gap:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <input style={{...S.input,marginTop:6,fontSize:13,padding:"8px 10px"}} type="number" step="0.25" min="0.25" placeholder="Outro valor..." value={[0.5,1,2,3].includes(parseFloat(form.units))?"":form.units} onChange={e=>setForm(f=>({...f,units:parseFloat(e.target.value)||1}))}/>
              </div>
            </div>
      ...
      <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:"#fff",borderBottom:"1px solid #f3f4f6",position:"sticky",top:0,zIndex:10,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {isInTrial && <span style={{borderRadius:6,padding:"3px 10px",fontSize:12,fontWeight:600,color:"#92400e",background:"#fef3c7",border:"1px solid #fde68a",cursor:"pointer"}} onClick={()=>setDrawerOpen(true)}>{trialLeft}d ⏰</span>}
      ...
      <main style={{maxWidth:680,margin:"0 auto",padding:"14px 12px"}}>
        ...
        {tab==="ai" && (
          <div>
            <div style={{background:"#fff",border:"1px solid #f3f4f6",borderRadius:14,padding:40,textAlign:"center",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
              ...
              <p style={{color:"#6b7280",fontSize:14,lineHeight:1.7,margin:0}}>
                Análise personalizada do teu histórico de {br?.sport} com score de saúde da banca, identificação dos melhores mercados, alertas de risco e recomendações.
              </p>
...
function AdminPanel({ supabase, fmt, daysLeft }) {
  ...
  useEffect(()=>{
    async function load(){
      setLoading(true);
      const{data:profiles}=await supabase.from("profiles").select("user_id,name,subscribed,plan,trial_start,user_trial_start,bankroll,sport,created_at").order("created_at",{ascending:false});
      if(profiles){
        const users={};
