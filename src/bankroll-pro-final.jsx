        ...
        {data?.users.map(u=>{
          ...
          return (
            <div key={u.user_id} style={{padding:"12px 0",borderBottom:"1px solid #f9fafb"}}>
              ...
              <div style={{display:"flex",gap:6}}>
                {!u.subscribed ? (
                  <button style={{flex:1,padding:"6px 4px",borderRadius:8,border:"1px solid #bbf7d0",background:"#f0fdf4",color:"#15803d",cursor:"pointer",fontSize:11,fontWeight:700}}
                    onClick={async()=>{
                      if(!window.confirm("Dar acesso VIP a este utilizador?")) return;
                      await supabase.from("profiles").update({subscribed:true,plan:"vip"}).eq("user_id",u.user_id);
                      setData(d=>({...d,users:d.users.map(x=>x.user_id===u.user_id?{...x,subscribed:true,plan:"vip"}:x)}));
                    }}>
                    ⭐ VIP
                  </button>
                ) : (
                  <button style={{flex:1,padding:"6px 4px",borderRadius:8,border:"1px solid #fca5a5",background:"#fef2f2",color:"#dc2626",cursor:"pointer",fontSize:11,fontWeight:700}}
                    onClick={async()=>{
                      if(!window.confirm("Revogar acesso deste utilizador?")) return;
                      await supabase.from("profiles").update({subscribed:false,plan:null}).eq("user_id",u.user_id);
                      setData(d=>({...d,users:d.users.map(x=>x.user_id===u.user_id?{...x,subscribed:false,plan:null}:x)}));
                    }}>
                    🚫 Revogar
                  </button>
                )}
                <button style={{flex:1,padding:"6px 4px",borderRadius:8,border:"1px solid #fca5a5",background:"#fef2f2",color:"#dc2626",cursor:"pointer",fontSize:11,fontWeight:700}}
                  onClick={async()=>{
                    if(!window.confirm("APAGAR este utilizador e todos os seus dados? Irreversível!")) return;
                    await supabase.from("bets").delete().eq("user_id",u.user_id);
                    await supabase.from("profiles").delete().eq("user_id",u.user_id);
                    setData(d=>({...d,users:d.users.filter(x=>x.user_id!==u.user_id),total:d.total-1}));
                  }}>
                  🗑 Apagar
                </button>
              </div>
