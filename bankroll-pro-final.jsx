...
const SPORTS_CONFIG = {
  "Ténis":       { icon:"🎾", markets:["Vencedor do Jogo","Handicap Games","Total Games O/U","Set Winner","Total Sets O/U","Resultado Correto Sets","1º Set Vencedor","Total Games 1º Set","Handicap Sets","Dupla Hipótese","Tie-Break no Jogo","1º Break de Serviço","Jogo em Deuce","Total Aces O/U","Total Double Faults O/U","Outros"] },
  "Futebol":     { icon:"⚽", markets:["1X2","Dupla Hipótese","Over/Under Golos","BTTS","Handicap Asiático","Handicap Europeu","Marcador Correto","1º Marcador","Total Cantos","Total Cartões","Over/Under 1ª Parte","Resultado ao Intervalo","Outros"] },
  "Basquetebol": { icon:"🏀", markets:["1X2","Handicap","Over/Under","1º Quarto","Moneyline","Outros"] },
  "Hóquei":      { icon:"🏒", markets:["1X2","Handicap","Over/Under","Resultado Final","Outros"] },
  "Baseball":    { icon:"⚾", markets:["Moneyline","Run Line","Over/Under","1ª Entrada","Outros"] },
  "Rugby":       { icon:"🏉", markets:["1X2","Handicap","Over/Under","Primeira Tentativa","Outros"] },
  "MMA/UFC":     { icon:"🥊", markets:["Vencedor","Método de Vitória","Round","Over/Under Rounds","Vai a Decisão","Outros"] },
  "Outros":      { icon:"🎯", markets:["1X2","Handicap","Over/Under","Outros"] },
};
...
function rStyle(r) {
  const m = {
    WIN:     ["#0d2118","#6ee7b7","#059669"],
    LOSS:    ["#1f0d0d","#fca5a5","#dc2626"],
    VOID:    ["#1c1208","#fcd34d","#d97706"],
    CASHOUT: ["#0d1520","#93c5fd","#2563eb"],
    PENDING: ["#13111f","#c4b5fd","#7c3aed"],
  };
...
export default function App() {
  ...
  const emptyForm = {event:"",market:"Vencedor do Jogo",selection:"",odd:"",units:1,notes:""};
  ...
  async function switchBankroll(id) {
    setActiveBR(id); setBets([]); setDrawerOpen(false);
    await loadBets(id); setAppView("dashboard");
  }
  ...
  if(screen==="landing") return (
    <div style={S.root}>
      <header style={S.landingHeader}>
        <div style={S.logoWrap}><span>📊</span><span style={S.logoText}>BankrollPro</span></div>
        <button style={S.btnSm} onClick={()=>{setAuthMode("login");setScreen("auth");}}>Entrar</button>
      </header>
      <div style={S.hero}>
        <div style={S.heroTag}>Preço de lançamento · Oferta limitada</div>
        <h1 style={S.heroTitle}>Gestão mais<br/><span style={{color:"#9ca3af"}}>inteligente.</span></h1>
        ...
        <div style={S.pricingRow}>
          ...
          <div style={{...S.pCard,border:"1px solid #374151"}}>
            <div style={{fontSize:9,color:"#9ca3af",letterSpacing:1.5,fontWeight:800,textTransform:"uppercase",marginBottom:6}}>Melhor valor</div>
            <div style={S.pLabel}>Anual</div>
            <div style={S.pOld}>€22,99/ano</div>
            <div style={S.pPrice}>€19,99<span style={S.pPer}>/ano</span></div>
            <div style={{...S.pTag,color:"#6ee7b7"}}>Poupas €28</div>
          </div>
        </div>
        <div style={S.sportRow}>
          {SPORTS.slice(0,6).map(s=>(
            <div key={s} style={S.sportPill}>
        ...
        <div style={S.featList}>
          {["Até 3 bancas simultâneas","Análise com IA por desporto","Gestão por unidades dinâmicas","7 dias grátis — sem cartão"].map(f=>(
            <div key={f} style={S.featItem}><span style={{color:"#374151",marginRight:10}}>—</span>{f}</div>
          ))}
        </div>
        <button style={S.btnPrimary} onClick={()=>{setAuthMode("register");setScreen("auth");}}>Começar grátis</button>
      </div>
    </div>
  );
    ...
    <div style={S.root}>
      ...
      <div style={S.centeredWrap}>
        <div style={S.authCard}>
          <h2 style={S.authTitle}>{authMode==="login"?"Bem-vindo de volta":"Criar conta"}</h2>
          ...
          <button style={{...S.btnPrimary,marginTop:16}} onClick={handleAuth} disabled={loading}>
            {loading?"...":authMode==="login"?"Entrar":"Criar conta"}
          </button>
          <p style={S.switchTxt}>
            {authMode==="login"?"Ainda não tens conta? ":"Já tens conta? "}
            <span style={S.switchLink} onClick={()=>setAuthMode(m=>m==="login"?"register":"login")}>
    ...
    <div style={S.root}>
      <div style={S.centeredWrap}>
        <div style={S.authCard}>
          ...
          <div style={S.trialPill}>7 dias grátis ativados</div>
          ...
          <div style={S.sportGrid}>
            {SPORTS.map(s=>(
              <button key={s} style={{...S.sportBtn,...(newBRForm.sport===s?S.sportBtnActive:{})}}
                onClick={()=>setNewBRForm(f=>({...f,sport:s}))}>
                <span style={{fontSize:18}}>{SPORTS_CONFIG[s].icon}</span>
                <span style={{fontSize:10,color:newBRForm.sport===s?"#f1f5f9":"#4b5563"}}>{s}</span>
              </button>
            ))}
          </div>
          ...
          {newBRForm.bankroll&&<p style={S.hint}>1 unidade = <strong style={{color:"#d1d5db"}}>€{((parseFloat(newBRForm.bankroll)||0)*(parseFloat(newBRForm.unit_pct)||2)/100).toFixed(2)}</strong> · Recomendamos 1–2%</p>}
      ...
      <div style={S.centeredWrap}>
        <div style={S.authCard}>
          ...
          <div style={S.planCard}>
            {subView==="annual"&&<div style={{fontSize:10,color:"#9ca3af",letterSpacing:1,fontWeight:800,textTransform:"uppercase",marginBottom:4}}>Melhor valor</div>}
            ...
            {subView==="annual"&&<div style={{fontSize:11,color:"#6ee7b7",marginBottom:12}}>€1,67/mês · Poupas €28</div>}
            <button style={{...S.btnPrimary,marginTop:12}} onClick={()=>handleSubscribe(subView)}>Subscrever</button>
          </div>
          <button style={{...S.btnGhost,marginTop:8}} onClick={()=>supabase.auth.signOut()}>Terminar sessão</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={S.root} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        ...
        <div style={S.overlay} onClick={()=>setDrawerOpen(false)}>
          <div style={S.drawer} onClick={e=>e.stopPropagation()}>
            ...
            <div style={S.drawerHeader}>
              ...
              <button style={{background:"none",border:"none",color:"#6b7280",fontSize:22,cursor:"pointer"}} onClick={()=>setDrawerOpen(false)}>×</button>
            </div>

            {/* Trial/plan status */}
            <div style={S.drawerStatus}>
              <span style={{fontSize:11,color:currentBankroll?.subscribed?"#6ee7b7":trialLeft>0?"#fcd34d":"#fca5a5",fontWeight:700}}>
                {currentBankroll?.subscribed?"✓ Plano ativo":trialLeft>0?`⏳ Trial — ${trialLeft} dias restantes`:"⚠️ Trial expirado"}
              </span>
            </div>

            {/* Bancas */}
            <div style={S.drawerSection}>Bancas</div>
            {bankrolls.map(br=>(
              <button key={br.id} style={{...S.drawerItem,...(br.id===activeBR?S.drawerItemActive:{})}}
                onClick={()=>switchBankroll(br.id)}>
                <span style={{fontSize:20}}>{SPORTS_CONFIG[br.sport]?.icon}</span>
                ...
                <div style={{fontSize:13,fontWeight:700,color:"#d1d5db"}}>{fmtE(parseFloat(br.bankroll))}</div>
              </button>
            ))}

            {/* Nova banca */}
            {bankrolls.length<MAX_BANKROLLS&&(
              <button style={S.drawerNewBR} onClick={()=>{setShowNewBR(true);setDrawerOpen(false);}}>
            ...
            {!currentBankroll?.subscribed&&(
              <div style={S.drawerSub}>
                <div style={{fontSize:11,color:"#6b7280",marginBottom:8}}>Subscreve para acesso ilimitado</div>
                <div style={{display:"flex",gap:8}}>
                  <button style={{...S.btnPrimary,fontSize:12,padding:"10px"}} onClick={()=>handleSubscribe("monthly")}>€3,99/mês</button>
                  <button style={{...S.btnPrimary,fontSize:12,padding:"10px",borderColor:"#374151"}} onClick={()=>handleSubscribe("annual")}>€19,99/ano</button>
                </div>
              </div>
            )}

            {/* Logout */}
            <button style={{...S.btnGhost,marginTop:12,fontSize:12}} onClick={()=>supabase.auth.signOut()}>
              Terminar sessão
            </button>
          </div>
        </div>
      ...
      {showNewBR&&(
        <div style={S.overlay}>
          <div style={S.modal}>
            ...
            <input style={S.input} type="number" step="0.5" min="0.5" max="10" value={newBRForm.unit_pct} onChange={e=>setNewBRForm(f=>({...f,unit_pct:e.target.value}))}/>
            {newBRForm.bankroll&&<p style={S.hint}>1 unidade = <strong style={{color:"#d1d5db"}}>€{((parseFloat(newBRForm.bankroll)||0)*(parseFloat(newBRForm.unit_pct)||2)/100).toFixed(2)}</strong></p>}
      ...
      <header style={S.header}>
        ...
        <div style={S.headerCenter}>
          <span style={{fontSize:16,marginRight:6}}>{SPORTS_CONFIG[currentBankroll?.sport||"Ténis"]?.icon}</span>
          <span style={S.headerTitle}>{currentBankroll?.name||"BankrollPro"}</span>
        </div>
        <div style={S.brChipWrap}>
          {trialLeft>0&&!currentBankroll?.subscribed&&(
            <span style={S.trialChip}>{trialLeft}d</span>
          )}
          <span style={S.brChip}>{fmtE(currentBR)}</span>
        </div>
      </header>

      {/* Nav */}
      <nav style={S.nav}>
        {[["dashboard","Dashboard"],["nova","Registar"],["history","Histórico"],["feedback","Análise IA"],].map(([v,l])=>(
          <button key={v} style={{...S.navBtn,...(appView===v?S.navActive:{})}}
      ...
      <main style={S.main}>
            ...
            {pts.length>1&&(
              <div style={S.card}>
                ...
                <svg viewBox={`0 0 ${svgW} ${svgH}`} style={{width:"100%",height:70,display:"block"}}>
                  ...
                  <polyline points={polyline} fill="none" stroke="#6b7280" strokeWidth="1.5" strokeLinejoin="round"/>
                  <line x1="0" y1={toY(currentBankroll?.bankroll||0)} x2={svgW} y2={toY(currentBankroll?.bankroll||0)} stroke="#1f2937" strokeWidth="1" strokeDasharray="4,3"/>
                </svg>
              </div>
            )}
                ...
                {bets.filter(b=>b.result==="PENDING").map(b=>(
                  <div key={b.id} style={S.pendRow}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:600,color:"#e5e7eb",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{b.selection}</div>
                      <div style={{fontSize:11,color:"#4b5563"}}>{b.market} · @{b.odd.toFixed(2)} · {fmtE(b.stake)}</div>
                    </div>
        ...
        {appView==="nova"&&(
          <div style={S.card}>
            <div style={S.cardTitle}>{SPORTS_CONFIG[currentBankroll?.sport||"Ténis"]?.icon} Novo Registo</div>
            ...
            <div style={{display:"flex",gap:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",gap:4,marginTop:4}}>
                  {[0.5,1,2,3].map(u=>(
                    <button key={u} style={{...S.unitBtn,...(parseFloat(form.units)===u?S.unitBtnActive:{})}}
            ...
            {form.odd&&parseFloat(form.odd)>1&&(
              <div style={S.stakeBox}>
                ...
                <span style={{marginLeft:"auto"}}>Retorno: <strong style={{color:"#6ee7b7"}}>{fmtE(unitVal*(parseFloat(form.units)||1)*parseFloat(form.odd))}</strong></span>
              </div>
            )}
            <label style={S.label}>Notas (opcional)</label>
            <input style={S.input} placeholder="Raciocínio, contexto..." value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
            <button style={{...S.btnPrimary,marginTop:18}} onClick={addBet}>Registar</button>
          </div>
        )}
            ...
            {filteredBets.map(b=>{
              ...
              return (
                <div key={b.id} style={S.betCard}>
                  <div style={{display:"flex",gap:10}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:14,fontWeight:600,color:"#f1f5f9",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{b.event}</div>
                      <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>{b.market} · <strong style={{color:"#9ca3af"}}>{b.selection}</strong></div>
                      {b.notes&&<div style={{fontSize:11,color:"#374151",fontStyle:"italic",marginTop:2}}>"{b.notes}"</div>}
                      <div style={{fontSize:10,color:"#1f2937",marginTop:4}}>{new Date(b.created_at).toLocaleDateString("pt-PT")}</div>
                    </div>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,minWidth:76,flexShrink:0}}>
                  ...
                  {b.result==="PENDING"&&(
                    <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
                      ...
                      <button style={S.bCash} onClick={()=>{const v=parseFloat(prompt("Valor do cashout (€):"));if(v>=0)settleBet(b.id,"CASHOUT",v);}}>Cashout</button>
        ...
        {appView==="feedback"&&(
          <div>
            <div style={S.card}>
              <div style={S.cardTitle}>Análise IA · {currentBankroll?.sport}</div>
              <p style={{color:"#6b7280",fontSize:13,lineHeight:1.6,marginBottom:16}}>Análise do teu histórico com recomendações personalizadas. Precisas de pelo menos 3 registos liquidados.</p>
              ...
              <div>
                <div style={{...S.card,textAlign:"center"}}>
                  <div style={S.scoreRing}>
                    ...
                    <div style={{fontSize:10,color:"#4b5563"}}>/10</div>
                  </div>
                  <div style={{fontSize:15,fontWeight:600,color:"#f1f5f9",marginTop:12}}>{feedback.headline}</div>
                </div>
                {feedback.warnings?.length>0&&(
                  <div style={{...S.card,background:"#1c1208",borderColor:"#78350f"}}>
                    <div style={{...S.cardTitle,color:"#fcd34d"}}>Alertas</div>
                ...
                <div style={S.card}>
                  ...
                  {feedback.insights?.map((ins,i)=>(
                    <div key={i} style={{display:"flex",padding:"8px 0",borderBottom:"1px solid #1f2937"}}>
                      <span style={{color:"#374151",marginRight:10,flexShrink:0}}>—</span>
...
function KPI({label,value,sub,valueColor,subColor}) {
  return (
    <div style={S.kpiCard}>
      ...
      {sub&&<div style={{fontSize:11,marginTop:3,color:subColor||"#4b5563"}}>{sub}</div>}
    </div>
  );
}
function Pill({label,value,color}) {
  return (
    <div style={S.pill}>
      <div style={{fontSize:10,color:"#374151",textTransform:"uppercase",letterSpacing:.8,marginBottom:3}}>{label}</div>
...
const S = {
  ...
  btnSm:{ background:"transparent",border:"1px solid #1f2937",color:"#6b7280",borderRadius:8,padding:"7px 14px",fontSize:12,cursor:"pointer",fontFamily:"inherit" },
  hero:{ padding:"32px 20px 60px",maxWidth:460,margin:"0 auto" },
  heroTag:{ display:"inline-block",background:"#161b22",border:"1px solid #21262d",color:"#6b7280",borderRadius:4,padding:"3px 10px",fontSize:10,fontWeight:700,letterSpacing:.5,textTransform:"uppercase",marginBottom:20 },
  heroTitle:{ fontSize:40,fontWeight:900,lineHeight:1.05,letterSpacing:"-2px",margin:"0 0 14px",color:"#f9fafb" },
  ...
  pLabel:{ fontSize:10,color:"#4b5563",textTransform:"uppercase",letterSpacing:.8,marginBottom:4 },
  pOld:{ fontSize:11,color:"#374151",textDecoration:"line-through",marginBottom:2 },
  pPrice:{ fontSize:22,fontWeight:900,color:"#f9fafb",letterSpacing:"-.5px" },
  pPer:{ fontSize:12,fontWeight:400,color:"#4b5563" },
  pTag:{ fontSize:10,color:"#6b7280",marginTop:4 },
  sportRow:{ display:"flex",flexWrap:"wrap",gap:8,marginBottom:20 },
  sportPill:{ display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:"#161b22",border:"1px solid #21262d",borderRadius:10,padding:"8px 10px",minWidth:52 },
  ...
  switchTxt:{ fontSize:12,color:"#374151",textAlign:"center",marginTop:12 },
  ...
  hint:{ fontSize:12,color:"#374151",margin:"6px 0 0" },
  planToggle:{ display:"flex",gap:4,background:"#0f1117",padding:4,borderRadius:8,marginTop:8 },
  planBtn:{ flex:1,padding:"8px 10px",borderRadius:6,border:"none",background:"transparent",color:"#4b5563",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700 },
  planBtnActive:{ background:"#1f2937",color:"#e5e7eb" },
  planCard:{ background:"#0f1117",border:"1px solid #1f2937",borderRadius:12,padding:20,textAlign:"center",marginTop:12 },
  planPrice:{ fontSize:26,fontWeight:900,color:"#f9fafb",marginBottom:4,letterSpacing:"-.5px" },

  // Drawer
  overlay:{ position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:100,display:"flex" },
  drawer:{ width:280,maxWidth:"85vw",background:"#161b22",borderRight:"1px solid #21262d",height:"100%",display:"flex",flexDirection:"column",padding:20,overflowY:"auto" },
  drawerHeader:{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,paddingBottom:16,borderBottom:"1px solid #1f2937" },
  drawerStatus:{ background:"#0f1117",border:"1px solid #1f2937",borderRadius:8,padding:"8px 12px",marginBottom:20,textAlign:"center" },
  drawerSection:{ fontSize:10,color:"#374151",textTransform:"uppercase",letterSpacing:1,fontWeight:800,marginBottom:8 },
  drawerItem:{ display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 8px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",borderRadius:8,marginBottom:4,textAlign:"left" },
  drawerItemActive:{ background:"#1f2937" },
  drawerNewBR:{ display:"flex",alignItems:"center",width:"100%",padding:"10px 8px",border:"1px dashed #1f2937",background:"transparent",cursor:"pointer",fontFamily:"inherit",borderRadius:8,color:"#4b5563",fontSize:13,fontWeight:600,marginTop:4 },
  drawerSub:{ borderTop:"1px solid #1f2937",paddingTop:16,marginTop:16 },

  // Header
  header:{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:"#161b22",borderBottom:"1px solid #21262d",position:"sticky",top:0,zIndex:10 },
  menuBtn:{ display:"flex",flexDirection:"column",gap:4,background:"none",border:"none",cursor:"pointer",padding:"4px",borderRadius:6 },
  menuLine:{ width:20,height:2,background:"#6b7280",borderRadius:2 },
  headerCenter:{ display:"flex",alignItems:"center" },
  headerTitle:{ fontSize:14,fontWeight:700,color:"#f1f5f9",letterSpacing:"-.3px" },
  brChipWrap:{ display:"flex",alignItems:"center",gap:6 },
  trialChip:{ background:"#0d2118",border:"1px solid #065f46",borderRadius:6,padding:"3px 7px",fontSize:10,color:"#6ee7b7",fontWeight:700 },
  ...
  navBtn:{ flex:1,padding:"12px 4px",border:"none",borderBottom:"2px solid transparent",background:"transparent",color:"#374151",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700,whiteSpace:"nowrap" },
  navActive:{ color:"#9ca3af",borderBottomColor:"#4b5563",background:"#0f1117" },
  ...
  kpiLabel:{ fontSize:10,color:"#374151",textTransform:"uppercase",letterSpacing:.8,marginBottom:5 },
  ...
  cardTitle:{ fontSize:10,color:"#374151",textTransform:"uppercase",letterSpacing:1,marginBottom:14,fontWeight:800 },
  ...
  label:{ fontSize:11,color:"#4b5563",marginBottom:4,marginTop:10,display:"block",fontWeight:600 },
  ...
  unitBtn:{ flex:1,padding:"10px 0",border:"1px solid #1f2937",borderRadius:8,background:"#0f1117",color:"#374151",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700,marginTop:4 },
  unitBtnActive:{ background:"#1f2937",color:"#e5e7eb",borderColor:"#374151" },
  stakeBox:{ display:"flex",alignItems:"center",flexWrap:"wrap",gap:8,background:"#0f1117",border:"1px solid #1f2937",borderRadius:8,padding:"10px 12px",fontSize:13,color:"#4b5563",marginTop:8 },
  ...
  btnGhost:{ width:"100%",background:"transparent",border:"1px solid #1f2937",color:"#4b5563",borderRadius:8,padding:"12px",fontSize:13,cursor:"pointer",fontFamily:"inherit",marginTop:4 },
  filterRow:{ display:"flex",gap:6,marginBottom:12,flexWrap:"wrap" },
  filterBtn:{ padding:"4px 12px",borderRadius:20,border:"1px solid #1f2937",background:"transparent",color:"#374151",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:700 },
  filterActive:{ background:"#1f2937",color:"#9ca3af",borderColor:"#374151" },
  ...
  bDel:{ padding:"5px 8px",borderRadius:6,border:"1px solid #1f2937",background:"transparent",color:"#1f2937",cursor:"pointer",fontSize:12,fontFamily:"inherit",marginLeft:"auto" },
  ...
  modal:{ background:"#161b22",border:"1px solid #21262d",borderRadius:16,padding:24,width:"100%",maxWidth:380,maxHeight:"80vh",overflowY:"auto",margin:"auto" },
...
if(typeof document!=="undefined"){
  ...
  s.textContent=`@keyframes spin{to{transform:rotate(360deg)}} select option{background:#0f1117} *{-webkit-tap-highlight-color:transparent;box-sizing:border-box} input:focus,select:focus{border-color:#374151!important;outline:none}`;
