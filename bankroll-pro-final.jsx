...
const STRIPE_MONTHLY = "https://buy.stripe.com/test_00w00l2va5Im8SLd4JfUQ00";
const STRIPE_ANNUAL  = "https://buy.stripe.com/test_eVq8wRd9Ob2G0mfc0FfUQ01";
const TRIAL_DAYS     = 7;
...
const CURRENCIES = {
  ...
  BRL: { symbol:"R$", label:"Real (R$)", flag:"🇧🇷" },
  USD: { symbol:"$", label:"Dólar ($)", flag:"🇺🇸" },
};
...
function useCurrency(code) {
  ...
  return { fmt, fmtP, fmtPct, symbol: cur.symbol };
}
...
export default function App() {
  ...
  const [editBRData, setEditBRData] = useState(null);
  ...
  const emptyForm = {event:"",market:"Vencedor do Jogo",selection:"",odd:"",units:1,result:"WIN",notes:"",cashoutVal:""};
  const [form, setForm]             = useState(emptyForm);
  const touchStartX                 = useRef(null);
  const { fmt, fmtP, fmtPct }      = useCurrency(currency);
  ...
  async function handleSetup(isEdit=false){
    const brv=parseFloat(newBRForm.bankroll);
    if(!brv||brv<=0||!newBRForm.name)return;
    if(isEdit&&editBRData){
      const updates={name:newBRForm.name,sport:newBRForm.sport,unit_pct:parseFloat(newBRForm.unit_pct)};
      if(newBRForm.resetBankroll) updates.bankroll=brv;
      const{data}=await supabase.from("profiles").update(updates).eq("id",editBRData.id).select().single();
      if(data){setBankrolls(prev=>prev.map(b=>b.id===data.id?data:b));setShowEditBR(false);}
    } else {
      const{data:{session}}=await supabase.auth.getSession();
      const uid=session?.user?.id||user?.id;
      const{data:existingP}=await supabase.from("profiles").select("user_trial_start,trial_start").eq("user_id",uid).order("created_at",{ascending:true});
      const earliestTrial=existingP?.[0]?.user_trial_start||existingP?.[0]?.trial_start||new Date().toISOString();
      const{data}=await supabase.from("profiles").insert({user_id:uid,name:newBRForm.name,sport:newBRForm.sport,bankroll:brv,unit_pct:parseFloat(newBRForm.unit_pct),trial_start:new Date().toISOString(),user_trial_start:earliestTrial,subscribed:false}).select().single();
  ...
  async function deleteBankroll(id){
    if(!window.confirm("Tens a certeza? Isto apaga a banca e todos os registos associados. Esta ação não pode ser revertida."))return;
  ...
  async function addBet(){
    ...
    if(editBet){
      const{data}=await supabase.from("bets").update({event:form.event,market:form.market,selection:form.selection,odd,stake,units:parseFloat(form.units),result,notes:form.notes,cashout_val:form.result==="CASHOUT"?parseFloat(form.cashoutVal)||null:null}).eq("id",editBet.id).select().single();
      if(data)setBets(prev=>prev.map(b=>b.id===data.id?{...data,odd:parseFloat(data.odd),stake:parseFloat(data.stake)}:b));
    } else {
      const{data}=await supabase.from("bets").insert({user_id:user.id,bankroll_id:activeBR,sport:br.sport,event:form.event,market:form.market,selection:form.selection,odd,stake,units:parseFloat(form.units),result,notes:form.notes,cashout_val:form.result==="CASHOUT"?parseFloat(form.cashoutVal)||null:null,created_at:new Date().toISOString()}).select().single();
  ...
  function openEditBet(b){
    setEditBet(b);
    setForm({event:b.event||"",market:b.market||"Vencedor do Jogo",selection:b.selection||"",odd:b.odd||"",units:b.units||1,result:b.result||"WIN",notes:b.notes||"",cashoutVal:b.cashout_val||""});
    setFormMode(b.result==="PENDING"?"pending":"immediate");
    setShowForm(true);
  }

  function openEditBR(b){
    setEditBRData(b);
    setNewBRForm({name:b.name,sport:b.sport,bankroll:b.bankroll,unit_pct:b.unit_pct,resetBankroll:false});
    setShowEditBR(true);
    setDrawerOpen(false);
  }
  ...
  const brHistory=useMemo(()=>{
    ...
    [...bets].reverse().filter(b=>b.result!=="PENDING").forEach(b=>{
      ...
      pts.push({v:r,date:b.created_at?.slice(0,10)});
    });
    return pts;
  },[bets,br]);
  ...
  if(screen==="landing")return(
    <div style={{...S.root,background:"#f9fafb"}}>
      ...
      {/* Hero */}
      <div style={S.hero}>
        <div style={S.heroTag}>🔥 Oferta de lançamento — Termina em breve</div>
        <h1 style={S.heroTitle}>Para de perder dinheiro.<br/><span style={{color:"#374151"}}>Começa a gerir.</span></h1>
        <p style={S.heroSub}>A ferramenta que os apostadores profissionais usam para controlar bancas, acompanhar ROI e tomar decisões com dados — não com emoção.</p>

        {/* Social proof */}
        <div style={S.socialProof}>
          <span style={{fontSize:20}}>⭐⭐⭐⭐⭐</span>
          <span style={{fontSize:12,color:"#6b7280",marginLeft:8}}>Gestão profissional ao alcance de todos</span>
        </div>

        {/* Pricing — promotional */}
        <div style={S.pricingBlock}>
          <div style={S.pricingHeader}>
            <span style={{fontSize:13,fontWeight:700,color:"#dc2626"}}>⏰ Preço de lançamento — só durante o trial</span>
          </div>
          <div style={S.pricingRow}>
            <div style={S.pCard}>
              ...
              <div style={{fontSize:11,color:"#dc2626",fontWeight:600,marginTop:4}}>-20% lançamento</div>
            </div>
            <div style={{...S.pCard,border:"2px solid #111827",boxShadow:"0 4px 16px rgba(0,0,0,.12)"}}>
              <div style={{fontSize:9,color:"#fff",background:"#111827",borderRadius:4,padding:"2px 8px",fontWeight:800,textTransform:"uppercase",letterSpacing:1,marginBottom:8,display:"inline-block"}}>MAIS POPULAR</div>
              <div style={S.pLabel}>Anual</div>
              <div style={S.pOld}>€22,99/ano</div>
              <div style={S.pPrice}>€19,99<span style={S.pPer}>/ano</span></div>
              <div style={{fontSize:11,color:"#059669",fontWeight:700,marginTop:4}}>Poupas €28 · €1,67/mês</div>
            </div>
          </div>
          <p style={{fontSize:11,color:"#9ca3af",textAlign:"center",margin:"8px 0 0"}}>Após o trial os preços voltam ao normal (€4,99/mês · €22,99/ano)</p>
        </div>

        {/* Features */}
        <div style={S.featGrid}>
          {[["📊","Múltiplas bancas","Até 3 bancas por desporto, separadas e independentes"],["🤖","Análise com IA","Feedback personalizado baseado no teu histórico real"],["📅","Diário & Relatório","Acompanha cada dia e cada mês com precisão"],["⚡","Registo rápido","Resultado imediato ou pendente — tu decides"],].map(([ico,t,d])=>(
            <div key={t} style={S.featCard}>
        ...
        <button style={{...S.btnPrimary,fontSize:16,padding:"16px",borderRadius:12,marginBottom:12}} onClick={()=>{setAuthMode("register");setScreen("auth");}}>
          🚀 Começar grátis — 7 dias
        </button>
        <button style={{...S.btnGhost,fontSize:13}} onClick={()=>{setAuthMode("login");setScreen("auth");}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center",marginTop:20}}>
          {SPORTS.map(s=>(
            <div key={s} style={{display:"flex",alignItems:"center",gap:4,background:"#fff",border:"1px solid #e5e7eb",borderRadius:20,padding:"4px 10px",fontSize:12}}>
              <span>{SPORTS_CONFIG[s].icon}</span><span style={{color:"#6b7280"}}>{s}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{...S.root,background:"#f9fafb"}}>
      ...
      <div style={S.centeredWrap}>
        <div style={S.authCard}>
          ...
          <p style={S.authSub}>{authMode==="login"?"Entra na tua conta.":"7 dias grátis. Preço de lançamento garantido."}</p>
    ...
    <div style={{...S.root,background:"#f9fafb"}}>
      <div style={S.centeredWrap}>
        <div style={S.authCard}>
          ...
          <div style={S.trialBanner}>🎯 Trial de 7 dias ativado · Preço de lançamento garantido</div>
          <BRForm form={newBRForm} setForm={setNewBRForm} showReset={false}/>
          <button style={{...S.btnPrimary,marginTop:20}} onClick={()=>handleSetup(false)}>Criar banca</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{...S.root,background:"#f9fafb"}}>
      <div style={S.centeredWrap}>
        <div style={S.authCard}>
          ...
          <h2 style={{...S.authTitle,textAlign:"center"}}>Trial terminado</h2>
          <p style={{...S.authSub,textAlign:"center"}}>Escolhe um plano para continuar a gerir a tua banca.</p>
          ...
          <div style={{...S.planCard,...(subView==="annual"?{border:"2px solid #111827"}:{})}}>
            {subView==="annual"&&<div style={{fontSize:11,color:"#059669",fontWeight:700,marginBottom:4,textAlign:"center"}}>Melhor valor · Poupas €28</div>}
            <div style={{...S.pOld,textAlign:"center"}}>{subView==="monthly"?"€4,99/mês":"€22,99/ano"}</div>
            <div style={{...S.planPrice,textAlign:"center"}}>{subView==="monthly"?"€3,99":"€19,99"}<span style={S.pPer}>{subView==="monthly"?"/mês":"/ano"}</span></div>
            <a href={subView==="monthly"?STRIPE_MONTHLY:STRIPE_ANNUAL} target="_blank" rel="noreferrer"
              style={{...S.btnPrimary,marginTop:16,display:"block",textAlign:"center",textDecoration:"none",lineHeight:"normal",padding:"13px"}}>
              Subscrever agora →
            </a>
          </div>

          <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"10px 12px",marginTop:12,fontSize:12,color:"#15803d",textAlign:"center"}}>
            ✓ Acesso imediato · ✓ Cancela quando quiseres · ✓ Suporte incluído
          </div>

          <button style={{...S.btnGhost,marginTop:8,fontSize:12}} onClick={()=>supabase.auth.signOut()}>Terminar sessão</button>
        </div>
      </div>
    </div>
  );
  ...
  const EditBRModal = showEditBR && (
    <div style={S.overlay}>
      <div style={{...S.modal,background:"#fff"}}>
        ...
        <BRForm form={newBRForm} setForm={setNewBRForm} showReset={true}/>
        <div style={{display:"flex",gap:8,marginTop:16}}>
          <button style={{...S.btnPrimary,flex:1}} onClick={()=>handleSetup(true)}>Guardar</button>
          <button style={{...S.btnPrimary,flex:0,padding:"13px 16px",background:"#fef2f2",color:"#dc2626",border:"1px solid #fca5a5"}}
            onClick={()=>{setShowEditBR(false);deleteBankroll(editBRData.id);}}>🗑</button>
        </div>
      </div>
    </div>
  );

  // ── MODAL: Register/Edit Bet ──
  const BetFormModal = showForm && (
    <div style={S.overlay}>
      <div style={{...S.modal,background:"#fff"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        {!editBet&&(
          <div style={S.modeToggle}>
            <button style={{...S.modeBtn,...(formMode==="immediate"?{background:sc?.color,borderColor:sc?.color,color:"#fff"}:{})}}
              onClick={()=>setFormMode("immediate")}>Resultado imediato</button>
            <button style={{...S.modeBtn,...(formMode==="pending"?{background:sc?.color,borderColor:sc?.color,color:"#fff"}:{})}}
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{display:"flex",gap:4,marginTop:4}}>
              {[0.5,1,2,3].map(u=>(
                <button key={u} style={{...S.unitBtn,...(parseFloat(form.units)===u?{background:sc?.color,borderColor:sc?.color,color:"#fff"}:{})}}
                  onClick={()=>setForm(f=>({...f,units:u}))}>{u}</button>
              ))}
            </div>
            <input
              style={{...S.input,marginTop:6,fontSize:13,padding:"8px 10px"}}
              type="number" step="0.25" min="0.25"
              placeholder="Outro valor..."
              value={[0.5,1,2,3].includes(parseFloat(form.units))?"":form.units}
              onChange={e=>setForm(f=>({...f,units:parseFloat(e.target.value)||1}))}/>
          </div>
        </div>
        ...
        {form.odd&&parseFloat(form.odd)>1&&(
          <div style={{...S.stakeBox,borderColor:sc?.color+"33",background:sc?.color+"08"}}>
            ...
            <span style={{marginLeft:"auto"}}>Retorno pot.: <strong style={{color:"#059669"}}>{fmt(unitVal*(parseFloat(form.units)||1)*parseFloat(form.odd))}</strong></span>
          </div>
        )}
          ...
          <>
            ...
            <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
              {[["WIN","✓ Green","#059669","#f0fdf4","#bbf7d0"],["LOSS","✗ Red","#dc2626","#fef2f2","#fca5a5"],["PENDING","⏳ Pendente","#7c3aed","#faf5ff","#c4b5fd"],["CASHOUT","💰 Cash","#2563eb","#eff6ff","#93c5fd"],["VOID","Void","#92400e","#fefce8","#fde68a"]].map(([r,l,c,bg,border])=>(
                <button key={r} style={{flex:1,minWidth:"30%",padding:"8px 4px",borderRadius:8,border:`1px solid ${form.result===r?c:border}`,background:form.result===r?bg:"#fff",color:form.result===r?c:"#9ca3af",cursor:"pointer",fontSize:11,fontWeight:700}}
                  onClick={()=>setForm(f=>({...f,result:r}))}>{l}</button>
              ))}
            </div>
            {form.result==="CASHOUT"&&(
              <><label style={S.label}>Valor cashout</label><input style={S.input} type="number" placeholder="ex: 12.50" value={form.cashoutVal} onChange={e=>setForm(f=>({...f,cashoutVal:e.target.value}))}/></>
            )}
          </>
        )}
        ...
        <button style={{...S.btnPrimary,marginTop:18,background:sc?.color,border:"none"}} onClick={addBet}>
          {editBet?"Guardar alterações":"Guardar registo"}
        </button>
      </div>
    </div>
  );
  ...
  return(
    <div style={{...S.root,background:"#f3f4f6"}} onTouchStart={swipeStart} onTouchEnd={swipeEnd}>
      ...
      {/* SUCCESS MODAL */}
      {showSuccess&&(
        <div style={{...S.overlay,alignItems:"center",justifyContent:"center"}} onClick={()=>setShowSuccess(false)}>
          <div style={{...S.modal,background:"#fff",textAlign:"center",maxWidth:360}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:56,marginBottom:12}}>🎉</div>
            <h2 style={{fontSize:22,fontWeight:900,color:"#111827",margin:"0 0 8px",letterSpacing:"-.5px"}}>Bem-vindo ao BankrollPro!</h2>
            <p style={{fontSize:14,color:"#6b7280",lineHeight:1.6,marginBottom:20}}>A tua subscrição está ativa. Já podes usar todas as funcionalidades sem limites.</p>
            ...
            <button style={{...S.btnPrimary,background:sc?.color,border:"none"}} onClick={()=>setShowSuccess(false)}>
              Começar agora →
            </button>
          </div>
        </div>
      ...
      {drawerOpen&&(
        <div style={S.overlay} onClick={()=>setDrawerOpen(false)}>
          <div style={S.drawer} onClick={e=>e.stopPropagation()}>
            ...
            {/* Trial banner in drawer */}
            {isInTrial&&(
              <div style={{...S.drawerTrial,background:"#fef3c7",border:"1px solid #fde68a",color:"#92400e"}}>
                <div style={{fontWeight:700,marginBottom:4}}>⏰ Trial — {trialLeft} dias restantes</div>
                <div style={{fontSize:11,marginBottom:8}}>Preço de lançamento garantido se subscreveres agora</div>
                <div style={{display:"flex",gap:6}}>
                  <a href={STRIPE_MONTHLY} target="_blank" rel="noreferrer" style={{flex:1,background:"#fff",border:"1px solid #fde68a",color:"#92400e",borderRadius:6,padding:"7px 0",fontSize:11,fontWeight:700,textAlign:"center",textDecoration:"none"}}>€3,99/mês</a>
                  <a href={STRIPE_ANNUAL} target="_blank" rel="noreferrer" style={{flex:1,background:"#111827",color:"#fff",borderRadius:6,padding:"7px 0",fontSize:11,fontWeight:700,textAlign:"center",textDecoration:"none"}}>€19,99/ano</a>
                </div>
              </div>
            )}

            {/* Currency */}
            <div style={S.drawerLabel}>Moeda</div>
            <div style={{display:"flex",gap:6,marginBottom:16}}>
              {Object.entries(CURRENCIES).map(([code,cur])=>(
                <button key={code} style={{flex:1,padding:"7px 4px",border:`1px solid ${currency===code?"#111827":"#e5e7eb"}`,background:currency===code?"#111827":"#fff",color:currency===code?"#fff":"#6b7280",borderRadius:8,cursor:"pointer",fontSize:11,fontWeight:700}}
                  onClick={()=>setCurrency(code)}>{cur.flag} {code}</button>
              ))}
            </div>
            ...
            {bankrolls.map(b=>{
              ...
              return(
                <div key={b.id} style={{...S.drawerItem,...(b.id===activeBR?{...S.drawerItemActive,borderLeftColor:bsc?.color}:{})}}>
                  <button style={{display:"flex",alignItems:"center",gap:10,flex:1,background:"none",border:"none",cursor:"pointer",padding:0,textAlign:"left"}}
                  ...
                  <button style={{background:"none",border:"none",color:"#d1d5db",cursor:"pointer",padding:"0 4px",fontSize:14,marginLeft:4}}
                    onClick={()=>openEditBR(b)}>✏️</button>
                </div>
              );
            })}

            {bankrolls.length<MAX_BANKROLLS&&(
              <button style={S.drawerAdd} onClick={()=>{setNewBRForm({name:"",sport:"Ténis",bankroll:"",unit_pct:"2"});setShowNewBR(true);setDrawerOpen(false);}}>
            ...
            {br?.subscribed&&(
              <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"10px 12px",marginTop:16,fontSize:12,color:"#15803d",fontWeight:600,textAlign:"center"}}>
                ✓ Plano ativo
              </div>
            )}
      ...
      {showNewBR&&(
        <div style={S.overlay}>
          <div style={{...S.modal,background:"#fff"}}>
            ...
            <BRForm form={newBRForm} setForm={setNewBRForm} showReset={false}/>
            <button style={{...S.btnPrimary,marginTop:16}} onClick={()=>handleSetup(false)}>Criar banca</button>
          </div>
        </div>
      )}
      ...
      <header style={S.header}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {isInTrial&&<span style={{...S.chip,color:"#92400e",background:"#fef3c7",border:"1px solid #fde68a",cursor:"pointer"}} onClick={()=>setDrawerOpen(true)}>{trialLeft}d ⏰</span>}
          <span style={{...S.chip,color:sc?.color,background:sc?.color+"15",border:`1px solid ${sc?.color}33`,fontWeight:800}}>{fmt(currentBR)}</span>
        </div>
      </header>

      {/* Trial urgency bar */}
      {isInTrial&&trialLeft<=3&&(
        <div style={{background:"#fef3c7",borderBottom:"1px solid #fde68a",padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:12,color:"#92400e",fontWeight:700}}>⚠️ Trial termina em {trialLeft} {trialLeft===1?"dia":"dias"} — preço de lançamento garantido agora</span>
          <a href={STRIPE_ANNUAL} target="_blank" rel="noreferrer" style={{fontSize:11,fontWeight:800,color:"#111827",textDecoration:"none",background:"#111827",color:"#fff",padding:"4px 10px",borderRadius:6}}>Subscrever</a>
        </div>
      )}

      {/* NAV */}
      <nav style={S.nav}>
        {[["dashboard","Banca"],["diary","Diário"],["report","Relatório"],["chart","Gráfico"],["ai","IA"]].concat(isAdmin?[["admin","Admin"]]:[] ).map(([v,l])=>(
      ...
      <main style={S.main}>
                ...
                {bets.filter(b=>b.result==="PENDING").map(b=>(
                  <div key={b.id} style={S.pendRow}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      ...
                      <button style={{...S.bDel,fontSize:14}} onClick={()=>openEditBet(b)}>✏️</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {tab==="diary"&&(
          <div>
            <div style={S.dateNav}>
              <button style={S.dateBtn} onClick={()=>{
                ...
                setDiaryDate(yy+"-"+mm+"-"+dd);
              }}>‹</button>
              <div style={{flex:1,textAlign:"center",fontSize:13,fontWeight:700,color:"#111827"}}>
                {(()=>{
                  const [y,m,d]=diaryDate.split("-").map(Number);
                  const dt=new Date(y,m-1,d);
                  return dt.toLocaleDateString("pt-PT",{weekday:"long",day:"numeric",month:"long"}).replace(/^\w/,c=>c.toUpperCase());
                })()}
              </div>
              <button style={S.dateBtn} onClick={()=>{
                const [y,m,d]=diaryDate.split("-").map(Number);
            ...
            <div style={{...S.card,padding:16,marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><div style={S.cardLabel}>Lucro do dia</div><div style={{fontSize:26,fontWeight:900,color:diaryPnl>=0?"#059669":"#dc2626",letterSpacing:"-1px"}}>{fmtP(diaryPnl)}</div></div>
                <div style={{textAlign:"right"}}><div style={S.cardLabel}>Registos</div><div style={{fontSize:26,fontWeight:900,color:"#374151"}}>{diaryBets.length}</div></div>
              </div>
            </div>
            ...
            {diaryBets.map(b=>{
              ...
              return(
                <div key={b.id} style={{...S.betCard,borderLeft:`3px solid ${borderColor}`}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
              {[["Banca Inicial",fmt(parseFloat(br?.bankroll||0))],["Banca Final",fmt(currentBR)],["Entradas",reportBets.length]].map(([l,v])=>(
                <div key={l} style={{...S.card,padding:12,textAlign:"center"}}><div style={S.cardLabel}>{l}</div><div style={{fontSize:14,fontWeight:800,color:"#111827"}}>{v}</div></div>
              ))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
              {[["Acertos",reportWins,"#059669"],["Erros",reportLoss,"#dc2626"],["% Acertos",reportWins+reportLoss>0?((reportWins/(reportWins+reportLoss))*100).toFixed(1)+"%":"—","#374151"]].map(([l,v,c])=>(
                <div key={l} style={{...S.card,padding:12,textAlign:"center"}}><div style={S.cardLabel}>{l}</div><div style={{fontSize:18,fontWeight:800,color:c}}>{v}</div></div>
              ))}
            </div>
          ...
          <div>
            <div style={{...S.card,padding:20}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginTop:12}}>
                <div style={{textAlign:"center"}}><div style={S.cardLabel}>Início</div><div style={{fontSize:14,fontWeight:700,color:"#374151"}}>{fmt(parseFloat(br?.bankroll||0))}</div></div>
                <div style={{textAlign:"center"}}><div style={S.cardLabel}>Atual</div><div style={{fontSize:14,fontWeight:700,color:currentBR>=parseFloat(br?.bankroll||0)?"#059669":"#dc2626"}}>{fmt(currentBR)}</div></div>
                <div style={{textAlign:"center"}}><div style={S.cardLabel}>Diferença</div><div style={{fontSize:14,fontWeight:700,color:stats.pnl>=0?"#059669":"#dc2626"}}>{fmtP(stats.pnl)}</div></div>
              </div>
            </div>
        ...
        {tab==="ai"&&(
          <div>
            <div style={{...S.card,textAlign:"center",padding:40}}>
              ...
              <div style={{fontSize:18,fontWeight:800,color:"#111827",marginBottom:8}}>Análise IA</div>
              <div style={{display:"inline-block",background:"#fef3c7",border:"1px solid #fde68a",borderRadius:20,padding:"4px 16px",fontSize:12,fontWeight:700,color:"#92400e",marginBottom:16}}>Em breve</div>
              <p style={{color:"#6b7280",fontSize:14,lineHeight:1.7,margin:0}}>
                A funcionalidade de análise com IA vai analisar o teu histórico de {br?.sport} e dar-te feedback personalizado sobre a tua gestão de banca, identificar padrões e sugerir melhorias.
              </p>
              <div style={{marginTop:24,display:"flex",flexDirection:"column",gap:10}}>
                {["Score de saúde da banca","Identificação dos melhores mercados","Alertas de risco","Recomendações personalizadas"].map(f=>(
                  <div key={f} style={{display:"flex",alignItems:"center",gap:10,background:"#f9fafb",border:"1px solid #f3f4f6",borderRadius:10,padding:"10px 14px"}}>
        ...
        {/* ── ADMIN ── */}
        {tab==="admin"&&isAdmin&&(
          <AdminPanel supabase={supabase} fmt={fmt}/>
        )}

      {/* FAB */}
      <button style={{...S.fab,background:sc?.color}} onClick={()=>{setForm(emptyForm);setEditBet(null);setShowForm(true);}}>+</button>
    </div>
  );
}

// ── Sub-component: Bankroll Form ──────────────────────────────────────────────
function BRForm({ form, setForm, showReset }) {
  const SPORTS_LIST = Object.keys(SPORTS_CONFIG);
  return (
    <>
      ...
      <div style={S.sportGrid}>
        {SPORTS_LIST.map(s=>(
          <button key={s} style={{...S.sportBtn,...(form.sport===s?{...S.sportBtnActive,borderColor:SPORTS_CONFIG[s].color,color:SPORTS_CONFIG[s].color}:{})}}
      ...
      <label style={S.label}>Bankroll {showReset?"(novo valor para repor)":""} (€)</label>
      ...
      {form.bankroll&&<p style={S.hint}>1 unidade = <strong>€{((parseFloat(form.bankroll)||0)*(parseFloat(form.unit_pct)||2)/100).toFixed(2)}</strong> · Recomendamos 1–2%</p>}
      {showReset&&(
        <label style={{...S.label,display:"flex",alignItems:"center",gap:8,marginTop:14,cursor:"pointer"}}>
          <input type="checkbox" checked={form.resetBankroll||false} onChange={e=>setForm(f=>({...f,resetBankroll:e.target.checked}))} style={{width:16,height:16}}/>
          <span style={{color:"#dc2626",fontWeight:600,fontSize:12}}>Repor bankroll para o valor acima</span>
        </label>
      )}
    </>
  );
...
function AdminPanel({ supabase, fmt }) {
  ...
  useEffect(()=>{
    async function load() {
      ...
      const { data: profiles } = await supabase
        .from("profiles")
        .select("user_id, name, subscribed, plan, trial_start, user_trial_start, bankroll, sport, created_at")
        .order("created_at", { ascending: false });

      if (profiles) {
        const uniqueUsers = {};
        profiles.forEach(p => {
          if (!uniqueUsers[p.user_id]) uniqueUsers[p.user_id] = { ...p, bancas: 1 };
          else uniqueUsers[p.user_id].bancas++;
        });
        const userList = Object.values(uniqueUsers);
        const paid = userList.filter(u => u.subscribed).length;
        const trial = userList.filter(u => !u.subscribed && daysLeft(u.user_trial_start || u.trial_start) > 0).length;
        const expired = userList.filter(u => !u.subscribed && daysLeft(u.user_trial_start || u.trial_start) === 0).length;
        setStats({ total: userList.length, paid, trial, expired });
        setUsers(userList);
      }
      setLoading(false);
    }
  ...
  if (loading) return <div style={{textAlign:"center",padding:40}}><div style={{width:28,height:28,border:"2px solid #e5e7eb",borderTop:"2px solid #6b7280",borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto"}}/></div>;
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        {[
          ["Total Utilizadores", stats?.total, "#111827"],
          ["Subscritores Pagos", stats?.paid, "#059669"],
          ["Em Trial", stats?.trial, "#d97706"],
          ["Trial Expirado", stats?.expired, "#dc2626"],
        ].map(([l,v,c])=>(
      ...
      <div style={{background:"#fff",border:"1px solid #f3f4f6",borderRadius:14,padding:16,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
        ...
        {users.map((u,i)=>{
          ...
          const statusColor = u.subscribed ? "#059669" : tl > 0 ? "#d97706" : "#dc2626";
          return (
            <div key={u.user_id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #f9fafb"}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:600,color:"#111827",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{u.user_id.slice(0,8)}...</div>
                <div style={{fontSize:11,color:"#9ca3af"}}>{u.bancas} banca{u.bancas>1?"s":""} · {u.plan||"sem plano"}</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0,marginLeft:10}}>
                <div style={{fontSize:12,fontWeight:700,color:statusColor}}>{status}</div>
                <div style={{fontSize:10,color:"#d1d5db"}}>{new Date(u.created_at).toLocaleDateString("pt-PT")}</div>
              </div>
            </div>
          );
        })}
        {users.length===0&&<div style={{textAlign:"center",color:"#9ca3af",padding:20}}>Nenhum utilizador ainda.</div>}
      </div>
    </div>
  );
}
...
const S={
  ...
  hero:{ padding:"24px 20px 80px",maxWidth:480,margin:"0 auto" },
  heroTag:{ display:"inline-block",background:"#fef3c7",border:"1px solid #fde68a",color:"#92400e",borderRadius:6,padding:"3px 12px",fontSize:11,fontWeight:700,marginBottom:16 },
  heroTitle:{ fontSize:36,fontWeight:900,lineHeight:1.05,letterSpacing:"-2px",margin:"0 0 12px",color:"#111827" },
  heroSub:{ fontSize:14,color:"#6b7280",lineHeight:1.7,marginBottom:20 },
  socialProof:{ display:"flex",alignItems:"center",marginBottom:20,background:"#fff",border:"1px solid #f3f4f6",borderRadius:10,padding:"10px 14px" },
  pricingBlock:{ background:"#fff",border:"1px solid #e5e7eb",borderRadius:14,padding:16,marginBottom:20,boxShadow:"0 2px 8px rgba(0,0,0,.06)" },
  pricingHeader:{ marginBottom:12,textAlign:"center" },
  pricingRow:{ display:"flex",gap:10,marginBottom:8 },
  pCard:{ flex:1,background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:12,padding:"12px 10px" },
  ...
  featGrid:{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24 },
  featCard:{ background:"#fff",border:"1px solid #f3f4f6",borderRadius:12,padding:"14px 12px",boxShadow:"0 1px 3px rgba(0,0,0,.04)" },
  ...
  drawerTop:{ display:"flex",alignItems:"center",gap:12,marginBottom:16,paddingBottom:16,borderBottom:"1px solid #f3f4f6" },
  ...
  drawerTrial:{ borderRadius:10,padding:"12px",marginBottom:14,fontSize:12 },
  drawerLabel:{ fontSize:10,color:"#9ca3af",textTransform:"uppercase",letterSpacing:1,fontWeight:800,marginBottom:8 },
  drawerItem:{ display:"flex",alignItems:"center",gap:4,width:"100%",padding:"10px 8px",borderLeft:"3px solid transparent",borderRadius:"0 10px 10px 0",marginBottom:4 },
  ...
  bWin:{ padding:"6px 12px",borderRadius:8,border:"1px solid #bbf7d0",background:"#f0fdf4",color:"#15803d",cursor:"pointer",fontSize:11,fontWeight:700 },
  bLoss:{ padding:"6px 12px",borderRadius:8,border:"1px solid #fca5a5",background:"#fef2f2",color:"#b91c1c",cursor:"pointer",fontSize:11,fontWeight:700 },
  bCash:{ padding:"6px 10px",borderRadius:8,border:"1px solid #93c5fd",background:"#eff6ff",color:"#1d4ed8",cursor:"pointer",fontSize:11,fontWeight:700 },
  bVoid:{ padding:"6px 10px",borderRadius:8,border:"1px solid #fde68a",background:"#fefce8",color:"#92400e",cursor:"pointer",fontSize:11,fontWeight:700 },
  ...
  fab:{ position:"fixed",bottom:24,right:18,width:56,height:56,borderRadius:"50%",border:"none",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(0,0,0,.2)",zIndex:20,fontSize:28,lineHeight:1 },
